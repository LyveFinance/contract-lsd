...
contract VaultGauge is IGauge {
    ...
    function claimFees() external lock returns (uint claimed0, uint claimed1) {
        require(false,"not support function") ;
    }

    function _claimFees() internal returns (uint claimed) {
        if (!isForPair) {
            return (0);
        }
        claimed = IVault(stake).claimFees();
        if (claimed > 0 ) {
            uint _fees = fees + claimed;
            address lsdToken = IVault(stake).lsdToken();
            if (_fees > IBribe(internal_bribe).left(lsdToken) ) {
                fees = 0;
                _safeApprove(lsdToken, internal_bribe, _fees);
                IBribe(internal_bribe).notifyRewardAmount(lsdToken, _fees);
            } else {
                fees = _fees;
            }
            emit ClaimFees(msg.sender, claimed,0);
        }
    }
